...
def extract_audio(storage_path: str, dest_dir: str = "/tmp") -> str:
    ...
    if result.returncode != 0:
        # ffmpeg writes diagnostics to stderr. Keep the tail only — it can be
        # verbose, and the last lines carry the actual failure reason. The
        # signed URL is in the command but ffmpeg doesn't echo it in stderr;
        # callers run this through sanitize_error before storing anyway.
