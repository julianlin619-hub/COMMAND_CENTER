...
export async function renderSquareQuoteCard(
  ...
  for (
    ...
    const totalBlockHeight =
      scaledH + (headerImg ? config.headerGap : 0) + textHeight;
  ...
  // 6. Vertically center the entire block (header + gap + text)
  const lineHeight = bestFontSize * lineHeightMult;
  const textHeight = calcTextHeight(bestLines, lineHeight);
  const totalBlockHeight =
    headerDrawHeight + (headerImg ? config.headerGap : 0) + textHeight;
