"""
X (Twitter) cron job entry point.
...
and this cron job reads them out, publishes them to X, and writes metrics back.

This script does two things each time it runs:
  Phase 1 - PUBLISH: Find posts that are due to be published and send them to X.
  Phase 2 - METRICS: For posts already published, pull engagement data (impressions,
            likes, retweets, replies, bookmark counts, etc.) from the X API and
...
# X is the platform adapter -- it implements the PlatformBase interface
# (create_post, get_post_metrics, refresh_credentials, etc.)
from platforms.x import X
...
def main():
    # Create the X platform client. This object knows how to talk to
    # the X (Twitter) API v2 -- posting tweets, fetching metrics, refreshing
    # tokens, etc.
    client = X()
    ...
    run_id = log_cron_start(platform="x", job_type="post")
    try:
        # OAuth 2.0 tokens expire after a set time (X access tokens last about
        # 2 hours). We MUST refresh credentials before making any API calls,
        ...
        # process_due_posts queries the database for X posts whose
        # scheduled_time has passed and status is still "scheduled", then
        # publishes each one via the X API. Returns the count of
        # posts that were processed.
        processed = process_due_posts(client, "x")
    ...
    run_id = log_cron_start(platform="x", job_type="metrics")
    try:
        # Fetch up to 50 posts that have already been published to X.
        # We limit to 50 to avoid hitting API rate limits -- X's API v2
        # has strict rate limits, especially on the free/basic tiers.
        posts = get_posts(platform="x", status="published", limit=50)
        for post in posts:
            # We check for platform_post_id because a post might be marked as
            # "published" in our database but not yet have an X tweet ID.
            # This can happen if the post was partially processed (e.g., tweeted
            # but the ID wasn't saved due to a crash). Without an X tweet ID,
            # we can't ask the API for metrics -- we wouldn't know which tweet
            # to ask about.
            if post.get("platform_post_id"):
                # Ask X's API for current engagement data (impressions, likes,
                # retweets, replies, bookmark counts, etc.) and store the
                # snapshot in our database.
...
# this file is executed directly (e.g., `python x_cron.py`), NOT when it's
