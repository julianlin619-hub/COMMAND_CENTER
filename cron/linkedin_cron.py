...
and this cron job reads them out, publishes them to LinkedIn, and writes metrics back.

This script does two things each time it runs:
  Phase 1 - PUBLISH: Find posts that are due to be published and send them to LinkedIn.
  Phase 2 - METRICS: For posts already published, pull engagement data (impressions,
            clicks, likes, comments, shares, etc.) from the LinkedIn API and store
            it in the database.
...
def main():
    ...
    try:
        # OAuth tokens expire after a set time (LinkedIn access tokens typically
        # last 60 days). We MUST refresh credentials before making any API calls,
        # otherwise we risk getting 401 Unauthorized errors. This exchanges our
        # stored refresh token for a fresh access token.
        client.refresh_credentials()

        # process_due_posts queries the database for LinkedIn posts whose
        # scheduled_time has passed and status is still "scheduled", then
        # publishes each one via the LinkedIn API. Returns the count of
    ...
    run_id = log_cron_start(platform="linkedin", job_type="metrics")
    try:
        # Fetch up to 50 posts that have already been published to LinkedIn.
        # We limit to 50 to avoid hitting API rate limits -- LinkedIn has daily
        # call quotas.
        posts = get_posts(platform="linkedin", status="published", limit=50)
        for post in posts:
            # We check for platform_post_id because a post might be marked as
            # "published" in our database but not yet have a LinkedIn post URN.
            # This can happen if the post was partially processed (e.g., created
            # on LinkedIn but the URN wasn't saved due to a crash). Without a
            # LinkedIn post URN, we can't ask the API for metrics -- we wouldn't
            # know which post to ask about.
            if post.get("platform_post_id"):
                # Ask LinkedIn's API for current engagement data (impressions,
                # clicks, likes, comments, shares, etc.) and store the snapshot
                # in our database.
