...
def _can_resend(post: dict, replay: dict | None) -> bool:
    ...
    """
    # All TikTok legs are retired (tweet-reel in Aug 2026, the carousel
    # mirror later that month) — never re-send a failed post back to the
    # TikTok channel. Returning False routes the row to the terminal
    # buffer_error path so it stays visible in the dashboard instead of
    # quietly re-queueing dead content.
