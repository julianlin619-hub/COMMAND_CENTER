...
and this cron job reads them out, publishes them to Instagram, and writes metrics back.

This script does two things each time it runs:
  Phase 1 - PUBLISH: Find posts that are due to be published and send them to Instagram.
  Phase 2 - METRICS: For posts already published, pull engagement data (likes, comments,
            reach, impressions, etc.) from the Instagram API and store it in the database.
...
def main():
    ...
    try:
        # Fetch up to 50 posts that have already been published to Instagram.
        # We limit to 50 to avoid hitting API rate limits -- Instagram's Graph
        # API has call quotas per hour.
        ...
        for post in posts:
            ...
            # "published" in our database but not yet have an Instagram media ID.
            # This can happen if the post was partially processed (e.g., created
            # on Instagram but the ID wasn't saved due to a crash). Without an
            # Instagram media ID, we can't ask the API for metrics -- we wouldn't
            # know which post to ask about.
            if post.get("platform_post_id"):
                # Ask Instagram's API for current engagement data (likes, comments,
                # reach, impressions, etc.) and store the snapshot in our database.
